"""pyrefly runner — reveal_type injection -> `pyrefly check --output-format json` -> main_result.json.
...
Per snippet:
  ...
  3. Drop a minimal `pyrefly.toml` next to the file so pyrefly applies its
     default `basic` checks (without a config it suppresses everything).
  4. Run `pyrefly check --output-format json transformed.py`.
  5. Walk the `errors` array; for each entry with `name == "reveal-type"`
     and `severity == "info"`, strip the ``revealed type: `` prefix from
     `description` and flatten pyrefly's type string onto TypeEvalPy's
     flat vocabulary.
  6. Write main_result.json next to main.py.

We never modify main.py. The transformed file lives in /tmp/pyrefly_work/
so multiple parallel pyrefly invocations stay isolated.
"""
...
logger = logging.getLogger("pyrefly-runner")
...
PYREFLY_BIN = "pyrefly"
PYREFLY_CONFIG = 'project-includes = ["**/*.py"]\n'

# pyrefly emits diagnostics with description shape:
#   "revealed type: <T>"
# stripped here.
REVEALED_PREFIX = "revealed type: "
...
def main_runner(benchmark_path: str) -> int:
    ...
    logger.info(f"pyrefly runner sweeping {len(gt_files)} snippets under {root}")
    processed = 0
    errors = 0
...
def process_snippet(snippet_dir: Path) -> None:
    ...
    work_root = Path(tempfile.mkdtemp(prefix="pyrefly_work_"))
    try:
        ...
        (work_dir / "pyrefly.toml").write_text(PYREFLY_CONFIG)
        payload = run_pyrefly(work_main)
    finally:
        shutil.rmtree(work_root, ignore_errors=True)

    predictions = parse_pyrefly_payload(payload, line_to_gt, gt, synthetic)
...
def plan_insertion(
    ...
    if kind == "return":
        ...
        if not fn.returns:
            ...
            return [Insertion(after_line=-1, indent="", expr=synth_expr, gt_index=gt_index)]
        out: list[Insertion] = []
...
def run_pyrefly(path: Path) -> dict:
    """Run pyrefly on a single file in JSON mode. Returns the parsed payload."""
    proc = subprocess.run(
        [PYREFLY_BIN, "check", "--output-format", "json", str(path)],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True,
    )
    if not proc.stdout:
        return {"errors": []}
    # pyrefly may append "INFO N errors" log lines after the JSON; isolate the
    # JSON object by finding the matched braces.
    text = proc.stdout.strip()
    try:
        # Most invocations dump pure JSON; try the fast path first.
        return json.loads(text)
    except json.JSONDecodeError:
        pass
    # Fallback: take everything up to the matched closing brace at depth 0.
    depth = 0
        ...
        if ch == "{":
            ...
            if depth == 0:
                try:
                    ...
                    break
    logger.warning(f"pyrefly JSON parse failed for {path}; stderr: {proc.stderr[:200]}")
    return {"errors": []}


def parse_pyrefly_payload(
    payload: dict,
    line_to_gt: dict[int, int],
    gt: list[dict],
    synthetic: dict[int, list[str]],
    ...
    for diag in payload.get("errors", []):
        ...
        if not desc.startswith(REVEALED_PREFIX):
            continue
        py_line = diag.get("line")
        if py_line is None:
            continue
        gt_index = line_to_gt.get(py_line)
        if gt_index is None:
            continue
        types = flatten_pyrefly_type(desc[len(REVEALED_PREFIX):])
...
# ---------------------------------------------------------------------------
# Map pyrefly type strings -> TypeEvalPy flat vocabulary.
# Pyrefly's output format closely mirrors pyright's: `Literal[42]`, `int | str`,
# `() -> R`, etc.
# ---------------------------------------------------------------------------
...
def flatten_pyrefly_type(text: str) -> list[str]:
    text = text.strip()
    ...
    if len(parts) > 1:
        out: list[str] = []
        for p in parts:
            out.extend(flatten_pyrefly_type(p))
        return out
